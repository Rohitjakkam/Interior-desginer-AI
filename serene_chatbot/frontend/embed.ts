/**
 * Serene Design Studio Chatbot - Single File Embed Script (TypeScript)
...
(function(): void {
    ...
    const apiUrl: string = currentScript?.getAttribute('data-api-url') || 'http://localhost:8001';
    const position: 'left' | 'right' = (currentScript?.getAttribute('data-position') as 'left' | 'right') || 'left';
    const primaryColor: string = currentScript?.getAttribute('data-color') || '#C9A961';
    ...
    class SereneChatbotEmbed {
        ...
        private init(): void {
            ...
            this.widget = widget;
            this.toggle = widget.querySelector('.serene-chat-toggle') as HTMLButtonElement;
            this.window = widget.querySelector('.serene-chat-window') as HTMLDivElement;
            this.messagesEl = widget.querySelector('.serene-chat-messages') as HTMLDivElement;
            this.input = widget.querySelector('.serene-chat-input') as HTMLInputElement;
            this.send = widget.querySelector('.serene-chat-send') as HTMLButtonElement;
            this.close = widget.querySelector('.serene-chat-close') as HTMLButtonElement;

            this.toggle.onclick = (): void => this.toggleChat();
            this.close.onclick = (): void => this.closeChat();
            this.send.onclick = (): void => this.sendMessage();
            this.input.onkeypress = (e: KeyboardEvent): void => {
                if (e.key === 'Enter') this.sendMessage();
            };
            this.input.oninput = (): void => {
        ...
        private async sendMessage(text: string | null = null): Promise<void> {
        ...
        private addBotMsg(text: string, replies: string[] = []): void {
            const el = document.createElement('div');
            ...
            if (replies.length) {
                ...
                replies.forEach(r => {
                    ...
                    btn.textContent = r;
                    btn.onclick = (): void => { rpl.remove(); this.sendMessage(r); };
                    rpl.appendChild(btn);
                });
                this.messagesEl.appendChild(rpl);
            }
            this.scroll();
        ...
        private showLeadForm(): void {
            ...
            this.scroll();
            (document.getElementById('ls') as HTMLButtonElement).onclick = (): void => this.submitLead(form);
        }

        private async submitLead(form: HTMLDivElement): Promise<void> {
            const n = (document.getElementById('ln') as HTMLInputElement).value.trim();
            const m = (document.getElementById('lm') as HTMLInputElement).value.trim();
            const l = (document.getElementById('ll') as HTMLInputElement).value.trim();
            const h = (document.getElementById('lh') as HTMLSelectElement).value;
            const b = (document.getElementById('lb') as HTMLSelectElement).value;
